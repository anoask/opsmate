  ...
  return (
    <Card className="border-border bg-card shadow-black/20">
      <CardHeader className="flex flex-row items-center gap-2">
        ...
        <CardTitle className="text-base font-semibold">Alert Activity</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {isLoading ? (
          <div className="text-sm text-muted-foreground">Loading alert activity...</div>
        ) : items.length === 0 ? (
          <div className="text-sm text-muted-foreground">No recent alert activity.</div>
        ) : (
          items.map((item) => (
            <div
              key={item.id}
